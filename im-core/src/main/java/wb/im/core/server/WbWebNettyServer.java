...
import wb.im.core.handler.WbNettyWebMessageHandler;
...
@Slf4j
@Component
public class WbWebNettyServer implements Runnable {

    @Value("${im.web-server.port:}")
    private Integer serverPort;

    @Resource
    WbNettyWebMessageHandler wbNettyWebMessageHandler;
    ...
    @PostConstruct
    public void start() {
        log.info("WEB消息服务开始启动（{}）", serverPort);
    ...
    @PreDestroy
    public void destroy() {
        log.info("WEB消息服务开始关闭({})", serverPort);
        if (mainGroup != null) {
    ...
    public void run() {
        ...
        serverBootstrap.group(mainGroup, workerGroup)
                ...
                .childHandler(wbNettyWebMessageHandler)
        ;
        try {
